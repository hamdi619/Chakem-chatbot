...
class InsertNewFeedback(Action):
    ...
    def run(self,
        ...
        stmt = "INSERT INTO feedback_ecommerce (feedbackText) VALUES (?)"
...
class SelectProductInformation(Action):
    ...
    def run(self,
        ...
        linkCursor = link.cursor(prepared=True)
        stmt = "SELECT name, description, photo, price FROM products WHERE name like ?"
        ...
        if linkCursor.rowcount > 0:
            ...
            for i in stmtResult:
                ...
                # stmtResultRow = "- Product: " + i[0] + "; description: " + i[1] + "; image: " + i[2] + "; unit price: " + str(round(i[3], 2)) + "€. \n"
                # stmtResultRow = "✅ Product: " + i[0]  + "; unit price: " + str(round(i[3], 2)) + "Dhs. \n\nhttp://newhost/E/Ecomm/comm/images/"+ i[2] +"\n\n"
...
class UpdateUserFirstName(Action):
    ...
    def run(self,
        ...
        linkCursor = link.cursor(prepared=True)
        stmt = "UPDATE users SET firstname = ? WHERE id = ?"
...
class InsertNewProductSuggestion(Action):
    ...
    def run(self,
        ...
        linkCursor = link.cursor(prepared=True)
        stmt = "INSERT INTO suggestions (suggestion_text) VALUES (?)"
...
class InsertNewMailForArequest(Action):
    ...
    def run(self,
        ...
        linkCursor = link.cursor(prepared=True)
        stmt = "INSERT INTO custom_requests (customRequestEmailAdress, customRequestText) VALUES (?, ?)"
...
class AddNewCustomRequest(Action):
    ...
    def run(self,
        ...
        linkCursor = link.cursor(prepared=True)
        stmt = "UPDATE custom_requests SET customRequestText = ? WHERE id = ? "
